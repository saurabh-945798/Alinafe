...
const SectionHeader = ({ icon, title, subtitle }) => (
  <div className="flex items-start justify-between gap-4">
    <div className="flex items-start gap-3">
      <div className="w-10 h-10 rounded-2xl bg-white/70 backdrop-blur border border-teal-900/10 flex items-center justify-center shadow-sm">
        {icon}
      </div>
      <div>
        <h3 className="text-lg sm:text-xl font-extrabold text-[#004D40] leading-tight">{title}</h3>
        {subtitle ? <p className="text-sm text-gray-600 mt-0.5">{subtitle}</p> : null}
      </div>
    </div>

    {/* Thin gradient divider line */}
    <div className="hidden sm:block flex-1 mt-5 h-[2px] rounded-full bg-gradient-to-r from-[#009688]/0 via-[#009688]/30 to-[#009688]/0" />
  </div>
);
          ...
          >
            <div className="flex items-center justify-between px-5 py-4 border-b border-teal-900/10">
              <div className="flex items-center gap-2">
                ...
                <p className="font-bold text-[#004D40]">Proof Preview</p>
              </div>
              <button
                onClick={onClose}
            ...
            <div className="p-4 sm:p-6 bg-gradient-to-br from-white via-white to-[#E0F2F1]/40">
              {!fileUrl ? (
                <div className="h-56 grid place-items-center text-gray-500">No proof found</div>
              ) : isPdf ? (
                <div className="rounded-2xl overflow-hidden border border-teal-900/10 bg-white">
                  <object
                    data={fileUrl}
              ...
              {fileUrl ? (
                <div className="mt-4 flex justify-end">
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="px-4 py-2 rounded-xl bg-[#009688] text-white font-semibold hover:bg-[#00695C] shadow"
                  >
                    Open in new tab
                  </a>
                </div>
...
const AdminReportDetail = () => {
  ...
  if (loading) {
    return (
      <section
        className="min-h-screen bg-gradient-to-br from-[#E0F2F1] via-white to-[#F1F8E9] py-10 px-6"
        style={{
      ...
      >
        <div className="max-w-4xl mx-auto">
          ...
          <div className="bg-white/70 backdrop-blur-xl shadow-xl rounded-3xl p-6 sm:p-8 border border-teal-900/10 overflow-hidden relative">
            <div className="absolute top-0 left-0 w-60 h-60 bg-[#009688]/20 blur-[120px] rounded-full"></div>
            <div className="absolute bottom-0 right-0 w-72 h-72 bg-[#004D40]/20 blur-[130px] rounded-full"></div>

            {/* Sticky Action Bar Skeleton */}
            <div className="sticky top-0 z-20 -mx-6 sm:-mx-8 px-6 sm:px-8 py-4 bg-white/85 backdrop-blur border-b border-teal-900/10">
  ...
  const timelineSteps = [
    {
      ...
      icon: <Clock size={16} className="text-[#009688]" />,
      done: !!createdAt,
    },
    {
      key: "updated",
      ...
      icon: <CheckCircle size={16} className="text-[#009688]" />,
    ...
    },
    {
      ...
      icon: <Trash2 size={16} className="text-[#009688]" />,
      done: !!adDeletedAt,
    },
  ];
  ...
  return (
    ...
    >
      ...
      <button
        onClick={() => navigate(-1)}
        className="mb-5 inline-flex items-center gap-2 text-[#004D40] hover:text-[#009688]"
      >
      ...
      <motion.div
        ...
        className={cx(
          "max-w-4xl mx-auto bg-white/70 backdrop-blur-xl shadow-xl rounded-3xl p-0 border border-teal-900/10 relative overflow-hidden",
          "border-l-4",
          statusMeta.border,
          statusMeta.glow
        )}
      >
        ...
        ===================================================== */}
        <div className="sticky top-0 z-30 bg-white/85 backdrop-blur border-b border-teal-900/10 px-6 sm:px-8 py-4">
          <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
            <div>
              <h2 className="text-2xl sm:text-3xl font-extrabold text-[#004D40] leading-snug">
        ...
        <div className="relative z-10 px-6 sm:px-8 py-8">
          ...
          >
            <div className="flex flex-wrap gap-2">
              <Chip
                icon={<ShieldAlert size={16} className="text-[#009688]" />}
                label="Reason"
                value={report.reason || "—"}
                className={cx("border", getReasonTone(report.reason))}
              />
              <Chip
                icon={<CheckCheck size={16} className="text-[#009688]" />}
                label="Status"
                value={statusMeta.label}
                className={cx("border", statusMeta.chip)}
              />
              <Chip
                icon={<MapPin size={16} className="text-[#009688]" />}
                label="Location"
                value={typeof ad.location === "string" ? ad.location : "—"}
                className="border border-teal-900/10"
              />
              <Chip
                icon={<IndianRupee size={16} className="text-[#009688]" />}
                label="Price"
                value={`₹ ${formatINR(ad.price)}`}
                className="border border-teal-900/10"
              />
              <Chip
                icon={<User size={16} className="text-[#009688]" />}
                label="Seller"
          ...
          <motion.div
            ...
            className="mb-6 bg-white/60 backdrop-blur-xl border border-teal-900/10 p-5 rounded-2xl shadow-sm"
          >
            <SectionHeader
              icon={<AlertTriangle size={18} className="text-[#009688]" />}
              title="Report Details"
              subtitle="Moderator context and user provided message"
            />

            <div className="mt-5">
              <h4 className="text-sm font-semibold text-[#004D40]">Message</h4>
              <p className="text-gray-800 mt-1 leading-relaxed">{report.message}</p>

              <div className="mt-4">
                <label className="text-sm font-semibold text-[#004D40] block mb-2">
                  Admin Note
                </label>
                <textarea
                  value={adminNote}
                  ...
                  rows={3}
                  className="w-full rounded-xl border border-teal-900/10 bg-white/80 px-3 py-2 text-sm text-gray-700 focus:outline-none focus:ring-2 focus:ring-[#009688]/30"
                />
              </div>
          ...
          <motion.div
            ...
            whileHover={{ y: -2 }}
            transition={{ type: "spring", stiffness: 260, damping: 22 }}
            className="mb-6 bg-white/60 backdrop-blur-xl border border-teal-900/10 rounded-2xl shadow-sm overflow-hidden"
          >
            <div className="p-5">
              <SectionHeader
                icon={<FileText size={18} className="text-[#009688]" />}
              ...
              <div className="mt-5 grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    {report.fileUrl ? (
                      <>
                        <button
                          ...
                          className="px-4 py-2 rounded-xl bg-[#009688] text-white font-semibold hover:bg-[#00695C] shadow inline-flex items-center gap-2"
                        >
                ...
                <motion.div
                  whileHover={{ y: -2 }}
                  className="rounded-2xl border border-teal-900/10 bg-white/70 backdrop-blur overflow-hidden shadow-sm"
                >
                  <div className="p-3 flex items-center gap-2 border-b border-teal-900/10">
                    <ImageIcon size={16} className="text-[#009688]" />
                    <p className="text-sm font-bold text-[#004D40]">Preview</p>
                  </div>
                  ...
                  <div className="p-3">
                    {!report.fileUrl ? (
                      ...
                      <motion.div
                        ...
                        className="rounded-xl overflow-hidden border border-teal-900/10"
                      >
                        <img
                          src={report.fileUrl}
                          alt="Proof thumbnail"
          ...
          >
            <SectionHeader
              icon={<Clock size={18} className="text-[#009688]" />}
              title="Timeline"
                ...
                <div key={step.key} className="flex items-start gap-3">
                  <div
                    className={cx(
                      ...
                      step.done ? "border-teal-900/10" : "border-gray-200"
                    )}
                  >
                    {step.icon}
                  </div>
          ...
          <motion.div
            ...
            initial="hidden"
            animate="show"
            className="border-t border-teal-900/10 pt-6 mt-6"
          >
            <SectionHeader
              icon={<ImageIcon size={18} className="text-[#009688]" />}
            ...
            <div className="mt-5 grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              <motion.div
                ...
                className="rounded-2xl overflow-hidden shadow-md border border-teal-900/10 bg-white/70 backdrop-blur"
              >
              ...
              <motion.div
                whileHover={{ y: -2 }}
                transition={{ type: "spring", stiffness: 240, damping: 20 }}
                className="md:col-span-2 bg-white/60 backdrop-blur-xl border border-teal-900/10 rounded-2xl p-5 shadow-sm"
              >
                ...
                <div className="mt-4 flex flex-wrap gap-2">
                  <PillTag
                    icon={<IndianRupee size={16} className="opacity-80" />}
                    text={`₹ ${formatINR(ad.price)}`}
                    className="border border-teal-900/10 bg-white/70"
                  />
                  <PillTag
                    icon={<MapPin size={16} className="opacity-80" />}
                    text={`Location: ${typeof ad.location === "string" ? ad.location : "—"}`}
                    className="border border-teal-900/10 bg-white/70 text-gray-700"
                  />
                  <PillTag
                    icon={<Tag size={16} className="opacity-80" />}
                ...
                <motion.div
                  whileHover={{ y: -2 }}
                  transition={{ type: "spring", stiffness: 240, damping: 20 }}
                  className="mt-5 p-4 bg-white/70 backdrop-blur-xl border border-teal-900/10 rounded-2xl shadow-sm"
                >
                  <div className="flex items-center justify-between gap-4">
                    <h4 className="text-sm font-semibold text-[#004D40] mb-2 flex items-center gap-2">
                      <User size={16} className="text-[#009688]" />
                      Seller Information
                    </h4>
                    <div className="h-[2px] flex-1 rounded-full bg-gradient-to-r from-[#009688]/0 via-[#009688]/25 to-[#009688]/0 mt-2" />
                  </div>
