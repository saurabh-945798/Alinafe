...
const BaseCategorySlider = ({ categoryTitle, category }) => {
  ...
  const getImageSrc = (img) =>
    img?.startsWith("http")
      ? img
      : `${BACKEND_ORIGIN}${String(img).replace(/\\/g, "/")}`;
  ...
  const getTopTags = (item) => {
    const tags = [];
    const categoryValue = String(item.category || "").trim();
    const categoryLower = categoryValue.toLowerCase();
    const subcategoryLower = String(item.subcategory || "").trim().toLowerCase();

    if (categoryLower === "real estate") {
      if (item.bedrooms)
        tags.push({ label: `${item.bedrooms} Beds`, key: "bedrooms" });
      if (item.bathrooms)
        tags.push({ label: `${item.bathrooms} Baths`, key: "bathrooms" });
      if (item.area) tags.push({ label: `${item.area} sq.ft`, key: "area" });
    } else if (categoryLower === "vehicles") {
      const isSpareParts = subcategoryLower.includes("spare parts");

      if (isSpareParts) {
        if (item.partCategory)
          tags.push({
            label: `Category ${item.partCategory}`,
            key: "partCategory",
          });
        if (item.originalType)
          tags.push({
            label: `Type ${item.originalType}`,
            key: "originalType",
          });
        if (item.workingStatus)
          tags.push({ 
            label: `Status ${item.workingStatus}`,
            key: "workingStatus",
          });
      } else {
        if (item.year) tags.push({ label: `Year ${item.year}`, key: "year" });
        if (item.mileage)
          tags.push({ label: `${item.mileage} km`, key: "mileage" });
      }
      // if (item.condition)
      //   tags.push({ label: item.condition, key: "condition" });
    } else if (categoryLower === "mobiles") {
      if (item.brand) tags.push({ label: ` ${item.brand}`, key: "brand" });
      if (item.model) tags.push({ label: `Model ${item.model}`, key: "model" });
      if (item.color) tags.push({ label: `Color ${item.color}`, key: "color" });
      if (item.warranty)
        tags.push({ label: `Warranty ${item.warranty}`, key: "warranty" });
      ...
      if (item.size) tags.push({ label: `Size ${item.size}`, key: "size" });
      if (item.color) tags.push({ label: item.color, key: "color" });
      // if (item.condition)
      //   tags.push({ label: item.condition, key: "condition" });
    } else if (categoryLower === "jobs") {
      if (item.salary) {
        const salaryValue = Number(item.salary);
        tags.push({
          label: `Salary/mo ₹ ${Number.isFinite(salaryValue)
            ? salaryValue.toLocaleString("en-MW")
            : item.salary}`,
          key: "salary",
        });
      }
      if (item.experience)
        tags.push({ label: `Experience ${item.experience}`, key: "experience" });
      if (item.company)
        tags.push({ label: `Company ${item.company}`, key: "company" });
    } else if (categoryLower === "agriculture") {
      if (item.quantity) tags.push({ label: item.quantity, key: "quantity" });
      // if (item.condition)
      //   tags.push({ label: item.condition, key: "condition" });
    } else if (categoryLower === "kids education") {
      if (item.ageGroup) tags.push({ label: item.ageGroup, key: "ageGroup" });
      // if (item.condition)
      //   tags.push({ label: item.condition, key: "condition" });
    } else if (categoryLower === "digital products") {
      if (item.fileType) tags.push({ label: item.fileType, key: "fileType" });
      if (item.accessType)
        tags.push({ label: item.accessType, key: "accessType" });
    } else if (categoryLower === "furniture") {
      if (item.type) tags.push({ label: item.type, key: "type" });
      if (item.color) tags.push({ label: item.color, key: "color" });
      if (item.size) tags.push({ label: item.size, key: "size" });
    } else if (categoryLower === "kitchenware & cookware") {
      if (item.utensilType)
        tags.push({ label: item.utensilType, key: "utensilType" });
      if (item.material) tags.push({ label: item.material, key: "material" });
      if (item.brand) tags.push({ label: item.brand, key: "brand" });
      // if (item.condition)
      //   tags.push({ label: item.condition, key: "condition" });
    } else if (categoryLower === "beauty & personal care") {
      if (item.productType)
        tags.push({ label: item.productType, key: "productType" });
      if (item.quantity) tags.push({ label: item.quantity, key: "quantity" });
      // if (item.condition)
      //   tags.push({ label: item.condition, key: "condition" });
      if (item.brand) tags.push({ label: item.brand, key: "brand" });
    } else if (categoryLower === "services") {
      const serviceTypeValue = item.serviceType || item.service || item.type;
      if (serviceTypeValue)
        tags.push({ label: `Service ${serviceTypeValue}`, key: "serviceType" });
      if (item.availability)
        tags.push({ label: `Availability ${item.availability}`, key: "availability" });
      const serviceAreaValue = item.serviceArea || item.area;
      if (serviceAreaValue)
        tags.push({ label: ` ${serviceAreaValue}`, key: "serviceArea" });
    } else if (categoryLower === "sports") {
      if (item.sportType) tags.push({ label: item.sportType, key: "sportType" });
      if (item.brand) tags.push({ label: `  ${item.brand}`, key: "brand" });
      if (item.size) tags.push({ label: `  ${item.size}`, key: "size" });
    } else if (categoryLower === "domain") {
      if (item.domainName) tags.push({ label: `Domain ${item.domainName}`, key: "domainName" });
      if (item.domainExtension)
        tags.push({ label: `Ext ${item.domainExtension}`, key: "domainExtension" });
      if (item.domainRegistrationYear)
        tags.push({ label: `Reg ${item.domainRegistrationYear}`, key: "domainRegistrationYear" });
    } else if (categoryLower === "pets & animals") {
      if (item.type) tags.push({ label: item.type, key: "type" });
      if (item.breed) tags.push({ label: item.breed, key: "breed" });
      if (item.age) tags.push({ label: item.age, key: "age" });
      if (item.gender) tags.push({ label: item.gender, key: "gender" });
    } else {
      if (item.brand) tags.push({ label: item.brand, key: "brand" });
    }

    // Density: show only top 2–3 fields
    return tags.slice(0, 3);
  };

  const getCategoryIcon = (cat) => {
    // lightweight, consistent icon mapping
    if (cat === "Vehicles") return <TrendingUp size={14} />;
    if (cat === "Electronics") return <Sparkles size={14} />;
    if (cat === "Real Estate") return <Tag size={14} />;
    if (cat === "Pets & Animals") return <Heart size={14} />;
    return <Tag size={14} />;
  };
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 -z-10">
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_20%_20%,rgba(13,148,136,0.18),transparent_55%),radial-gradient(circle_at_80%_10%,rgba(20,184,166,0.16),transparent_55%),radial-gradient(circle_at_60%_90%,rgba(8,145,178,0.12),transparent_55%)]" />
...
.alinafe-nav-btn:active{
      ...
      <div className="flex justify-between items-end gap-4 mb-6">
        ...
        <div className="shrink-0 flex items-center gap-3">
          ...
          >
            ...
            <span className="inline-flex items-center justify-center h-7 w-7 rounded-full bg-[color:var(--brand)] text-white shadow-[0_10px_24px_rgba(46,49,146,0.28)]">
              <ArrowRight size={16} />
            </span>
          </button>
        </div>
  ...
  sm:right-0 z-10"
          ...
          <Swiper
            ...
            spaceBetween={16} // thoda tight spacing for mobile
            loop
            allowTouchMove={true}
            simulateTouch={true}
            navigation={{
                  ...
                  <motion.div variants={cardV}>
                    ...
                    >
                      ...
                      <div className="absolute inset-0 pointer-events-none">
                        <div className="absolute -top-20 -right-20 h-56 w-56 rounded-full bg-[color:var(--brand)]/12 blur-3xl" />
                        <div className="absolute -bottom-24 -left-24 h-56 w-56 rounded-full bg-teal-400/12 blur-3xl" />
                      </div>
                      ...
                      <div className="p-4 md:p-5 flex flex-col justify-between min-h-[220px] relative z-10">
                        ...
                        <div className="mt-4">
                                ...
                                <>
                                  <p className="text-[22px] md:text-[24px] font-extrabold tracking-tight text-[color:var(--brand)] leading-none flex items-center gap-3">
                                    ...
                                    {["New", "Used"].includes(item.condition) &&
                                      !["Agriculture", "Jobs", "Jobs & Services", "Services", "Domain" , "Pets" , "Real Estate"].includes(item.category) && (
                          ...
                          <div className="mt-3 flex items-center justify-between gap-3 text-sm text-[color:var(--muted)]">
                            ...
                            <span className="text-[10px] bg-[#0D9488]/10 text-[#0D9488] px-2 py-0.5 rounded-full whitespace-nowrap inline-flex items-center gap-1">
                              <Clock size={10} />
