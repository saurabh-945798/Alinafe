...
const MessageBubble = ({
  ...
  // ✅ tighter spacing + grouping logic
  ...
  const meBubbleSurface =
    "bg-gradient-to-br from-[#009688] via-[#0E9F9F] to-[#00796B] text-white";
  const otherBubbleSurface =
    "bg-white/55 backdrop-blur-xl text-gray-900";

  /* 🧩 Asymmetric radius (chat-like) + grouping */
  const radiusMe = [
    "rounded-t-2xl",
  ...
  /* 🧱 Bubble base (responsive width + thinner borders) */
  const bubbleBase =
  ...
  const bubbleClass = isDeleted
    ? `${bubbleDeleted} ${isMe ? "bg-[#009688]/20 text-white/80 ring-white/20" : ""} ${
  ...
  /* ✅ meta badge inside bubble bottom-right */
    ...
    {type === "text" && (
      <p className="whitespace-pre-wrap break-words leading-relaxed text-[15px] sm:text-[16px]">
        {msg.message}
          ...
          {/* ✅ compact meta badge inside bubble (bottom-right) */}
