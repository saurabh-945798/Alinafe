        ...
        <div className="rounded-3xl bg-white/50 backdrop-blur-xl border border-white/60 shadow-[0_20px_60px_rgba(15,23,42,0.08)] p-8">
          <div className="flex items-center justify-center">
            <div className="relative">
              <div className="h-24 w-24 rounded-3xl bg-gradient-to-br from-[#0E9F9F]/20 via-[#3B82F6]/10 to-[#A855F7]/10 border border-white/60 backdrop-blur-xl shadow-inner" />
              ...
              <div className="absolute -left-3 -bottom-3 h-10 w-10 rounded-2xl bg-white/70 border border-white/60 backdrop-blur-xl flex items-center justify-center shadow">
                <Circle className="text-[#0E9F9F]" size={18} />
              </div>
            </div>
          </div>
          ...
          <p className="mt-2 text-sm text-slate-600 text-center leading-relaxed">
            Pick a conversation from the left panel to start chatting. You’ll see the ad context,
          ...
          <div className="mt-6 flex items-center justify-center">
            ...
            <div className="hidden lg:flex items-center gap-2 text-xs text-slate-500">
              <span className="inline-block h-2 w-2 rounded-full bg-[#0E9F9F]" />
...
/* ------------------------------------------
  Typing Indicator (dots + subtitle “typing…”)
------------------------------------------- */
  ...
  return (
    <div className="flex items-end gap-3 px-2 py-1">
      <div className="rounded-2xl bg-white/60 backdrop-blur-xl border border-white/60 shadow px-4 py-3">
        ...
        <div className="mt-2 text-[11px] text-slate-500">typing…</div>
      </div>
    </div>
  );
};
...
const Chats = () => {
  ...
  const fetchConvos = useCallback(async () => {
    ...
    try {
      ...
      const res = await axios.get(`${API_BASE}/conversations/${user.uid}`, authHeader);
      setConversations(res.data || []);
    } catch (e) {
      console.error("Conversation fetch error:", e);
    }
  }, [API_BASE, user?.uid, getAuthHeader]);
...
------------------------------------------- */
  useEffect(() => {
    const openFromUrl = async () => {
      ...
      const authHeader = await getAuthHeader();
      axios.put(
        `${API_BASE}/conversations/${chatToOpen.conversationId}/mark-read/${user.uid}`,
        {},
        authHeader
      );
    ...
    openFromUrl();
  }, [autoChatId, conversations, loadMessages, API_BASE, user?.uid, getAuthHeader]);
  ...
  const handleDeleteChat = async () => {
    ...
    try {
      const authHeader = await getAuthHeader();
      await axios.delete(`${API_BASE}/conversations/delete/${selectedChat.conversationId}`, {
        ...authHeader,
      });
  ...
  const openChat = async (chat) => {
    ...
    const authHeader = await getAuthHeader();
    axios.put(
      `${API_BASE}/conversations/${chat.conversationId}/mark-read/${user.uid}`,
      {},
      authHeader
    );
  ...
  return (
    <div className="min-h-screen font-[Poppins] bg-[#F8FAFC]">
      ...
      <div className="fixed inset-0 -z-10">
        <div className="absolute inset-0 bg-gradient-to-br from-[#0E9F9F]/10 via-[#3B82F6]/8 to-[#A855F7]/10" />
        <div className="absolute -top-40 -right-40 h-[420px] w-[420px] rounded-full bg-[#0E9F9F]/15 blur-3xl" />
          ...
          <div className="hidden lg:flex w-[360px] shrink-0 flex-col bg-white/40 backdrop-blur-xl border-r border-white/60">
            ...
            <div className="p-4 pb-3">
              <div className="relative">
                ...
                />
                <input
                  className="w-full rounded-2xl bg-white/70 backdrop-blur-xl border border-white/70 pl-11 pr-11 py-3 text-sm text-slate-800 placeholder:text-slate-500 shadow-sm focus:shadow-md focus:outline-none focus:ring-2 focus:ring-[#0E9F9F]/25"
              ...
              <motion.div variants={listWrap} initial="hidden" animate="show" className="space-y-1">
                <AnimatePresence initial={false}>
                  {filtered.map((chat) => {
                    ...
                    const lastMsg = safe(chat.lastMessage, safe(chat.lastMessageText, "Say hi…"));
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div className="min-w-0 flex-1">
                            ...
                            <div className="mt-0.5 flex items-center justify-between gap-2">
                              ...
                              {/* Unread badge pills */}
                              {unread > 0 && (
                                <span className="shrink-0 text-[11px] font-semibold px-2 py-0.5 rounded-full bg-[#0E9F9F]/15 text-[#0E9F9F] border border-white/70">
                                  {unread}
                                </span>
                              )}
                            </div>
                ...
                {selectedChat && (adPillTitle || adPillPrice) ? (
                  <div className="hidden sm:flex items-center gap-2 shrink-0">
                    <span className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-white/70 border border-white/70 backdrop-blur-xl shadow-sm">
                      <span className="h-2 w-2 rounded-full bg-[#0E9F9F]" />
                      <span className="text-[12px] font-medium text-slate-800 max-w-[180px] truncate">
                        {adPillTitle || "Ad"}
                      </span>
                      {adPillPrice && (
                        <span className="text-[12px] font-semibold text-[#0E9F9F]">
                          {adPillPrice}
                        </span>
                      )}
                    </span>
                    ...
                    <div className="flex items-center gap-4 p-3">
                      ...
                      {activeAd.adId && (
                        <button
                          ...
                          className="h-11 px-4 rounded-2xl bg-[#0E9F9F] text-white text-sm font-semibold shadow hover:shadow-md active:scale-[0.99] transition"
                        >
                          View Ad
                        </button>
                      )}
