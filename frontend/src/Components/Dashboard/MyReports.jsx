...
const MyReports = () => {
  ...
  useEffect(() => {
    ...
    const fetchReports = async () => {
      try {
        ...
        const res = await axios.get(`${API_BASE}/reports/user/${user.uid}`, {
  ...
  if (loading) {
    return (
      <section className="min-h-screen px-4 sm:px-6 py-14 bg-gradient-to-b from-[#F9FBFB] to-white lg:pl-[280px]">
  ...
  return (
    <section className="relative min-h-screen px-4 sm:px-6 py-14 font-[Poppins] text-gray-800 overflow-hidden lg:pl-[280px]">
      ...
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_top_left,rgba(0,150,136,0.08),transparent_40%),radial-gradient(circle_at_bottom_right,rgba(0,150,136,0.06),transparent_40%)]" />
      ...
      >
        ...
        <div className="flex flex-col sm:flex-row justify-between gap-4 mb-8">
          <h1 className="text-3xl font-semibold text-[#009688] flex items-center gap-2">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-8">
          {Object.entries(counts).map(([label, value]) => (
            <div
              key={label}
              className="rounded-xl px-4 py-3 bg-gradient-to-br from-white to-[#F1FDFB] border border-gray-100 text-center"
            >
              <p className="text-sm text-gray-500">{label}</p>
              <p className="text-2xl font-semibold text-[#009688]">
                {value}
              </p>
            </div>
          ))}
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-10">
          ...
          <div className="relative w-full md:w-72">
            ...
            <input
              ...
              className="w-full pl-9 pr-4 py-2 rounded-xl border border-gray-200 focus:outline-none focus:ring-2 focus:ring-[#009688]/30"
            />
          </div>
        </div>
        ...
        {filteredReports.length === 0 ? (
          <div className="flex flex-col items-center py-20 text-center">
            <div className="w-24 h-24 rounded-full bg-[#E0F2F1] flex items-center justify-center mb-4">
            ...
            <div className="flex gap-3">
              <button
                onClick={() => navigate("/")}
                className="px-5 py-2 rounded-xl bg-[#009688] text-white font-medium flex items-center gap-2"
              >
            ...
            <AnimatePresence>
              {filteredReports.map((report, index) => (
                <motion.div
                  ...
                  className="rounded-2xl border border-gray-100 bg-gradient-to-br from-white to-[#F9FAFA] p-5 hover:shadow-xl transition"
                >
                  <div className="flex justify-between gap-4">
                    {/* Left */}
                    <div className="space-y-2">
                      <h3 className="font-semibold text-[#009688]">
                        {report.adTitle}
                      </h3>
                  ...
                  {report.fileUrl && (
                    <a
                      ...
                      className="inline-flex items-center gap-2 mt-4 text-sm font-medium text-[#009688]"
                    >
