...
const ChatPreviewSection = () => {
  ...
  return (
    <Card className="relative overflow-hidden rounded-3xl border border-white/50 bg-white/60 backdrop-blur-xl shadow-xl p-6">
      {/* Gradient glow */}
      <div className="absolute -top-20 -right-20 w-64 h-64 bg-[#009688]/20 rounded-full blur-3xl" />

      {/* Header */}
      <div className="relative flex items-center justify-between mb-5">
        <h3 className="text-lg font-semibold text-[#004D40] flex items-center gap-2">
          <div className="p-2 rounded-xl bg-[#E0F2F1]">
            <MessageSquare className="w-5 h-5 text-[#009688]" />
          </div>
          Recent Chats
        </h3>
        ...
        <button
          ...
          className="text-sm font-medium text-[#009688] flex items-center gap-1 hover:underline"
        >
          ...
          {chats.map((chat) => {
            ...
            return (
              ...
              >
                ...
                {isUnread && (
                  <span className="absolute left-0 top-3 bottom-3 w-1 rounded-full bg-[#009688] animate-pulse" />
                )}
                ...
                <div className="relative">
                  ...
                  {/* Unread dot */}
                  {isUnread && (
                    <span className="absolute -top-1 -right-1 w-3 h-3 bg-[#009688] rounded-full animate-pulse" />
                  )}
                </div>
                ...
                {/* Unread count */}
                {isUnread && (
                  <Badge className="bg-[#009688] text-white rounded-full px-2.5 py-0.5 text-xs shadow-md">
                    {chat.unreadCount}
                  </Badge>
                )}
              </motion.button>
