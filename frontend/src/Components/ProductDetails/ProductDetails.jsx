...
const ProductDetails = () => {
  ...
  const descLines = description
    ...
    .map((line) => line.trim())
    .filter(Boolean);
  const descItems = descLines.map((line) =>
    line.replace(/^\d+\s*[.)-]?\s*/, "")
  );
...
useEffect(() => {
  ...
  useEffect(() => {
    const incrementView = async () => {
      ...
      try {
        const res = await axios.put(`${API_BASE}/ads/${id}/view`, {
    ...
    const fetchRelated = async () => {
      try {
        const res = await fetch(
          `${API_BASE}/ads?category=${encodeURIComponent(
            ad.category
          )}&limit=8`
        );
  ...
  if (loading)
    return (
      <div className="min-h-screen flex items-center justify-center text-[#0E9F9F] font-semibold text-xl animate-pulse">
  ...
  const renderCategoryFields = () => {
    if (["Services", "Pets"].includes(ad?.category)) return null;
    const normalizedCategory = (ad?.category || "").toLowerCase();
    const fields =
      CATEGORY_FIELDS[ad.category] ||
      (normalizedCategory.includes("sports") ? CATEGORY_FIELDS.Sports : null);
      ...
      <div className="mt-6 bg-white border border-gray-100 rounded-2xl shadow-sm p-6">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 text-gray-700">
          {ad?.condition &&
            ...
            !["Jobs", "Services", "Domain", "Real Estate"].includes(ad?.category) && (
          ...
          {fieldList.map((field) => {
            ...
            return (
              ...
              >
                {Icon && <Icon size={16} className="text-[#0E9F9F]" />}
  ...
  return (
    <section className="min-h-screen bg-gradient-to-br from-white to-[#E6FFFD] px-4 sm:px-10 pt-28 pb-32 font-[Poppins]">
      ...
      <motion.button
        ...
        whileHover={{ x: -5 }}
        className="flex items-center text-[#0E9F9F] mb-8 font-medium hover:underline"
      >
                ...
                return (
                  <img
                    ...
                    className={`w-20 h-20 rounded-lg border cursor-pointer object-cover transition ${
                      i === fullImageIndex
                        ? "border-[#0E9F9F] border-2"
                        : "border-gray-200 hover:border-[#0E9F9F]/50"
                    }`}
                  />
                );
              })
        ...
        <div className="bg-white rounded-3xl shadow-md p-8">
          ...
          <div className="bg-gradient-to-br from-white via-[#FAFEFE] to-[#F1FBFB] rounded-3xl p-6 shadow-[0_4px_20px_rgba(0,0,0,0.04)] mb-6 transition-all duration-300 hover:shadow-[0_6px_25px_rgba(0,0,0,0.08)]">
            ...
            {/* Price Block */}
              <div className="flex items-center justify-between flex-wrap gap-3">
                {ad.category !== "Services" && (
                  <p className="text-4xl font-extrabold text-[#0E9F9F] bg-[#0E9F9F]/5 px-4 py-2 rounded-2xl shadow-inner">
                    {ad.category === "Jobs"
                      ? "Jobs"
            ...
            <div className="mt-3 text-gray-500 text-sm flex items-center gap-2">
              <Clock className="text-[#0E9F9F]" size={15} />
              {ad.createdAt
          ...
          <div className="flex items-center gap-4 mb-6">
            <motion.button
              ...
              className={`relative px-6 py-2.5 rounded-full text-sm font-semibold border flex items-center gap-2 shadow-sm transition-all duration-300 ${
                isFav
                  ? "bg-gradient-to-r from-[#0E9F9F] to-[#10B5B5] text-white border-[#0E9F9F] shadow-[0_4px_15px_rgba(14,159,159,0.3)]"
                  : "border-[#0E9F9F] text-[#0E9F9F] hover:bg-[#0E9F9F] hover:text-white hover:shadow-[0_4px_15px_rgba(14,159,159,0.25)]"
              }`}
            >
              {/* Animated Heart Icon */}
              <motion.div
          ...
          <div className="mt-8 bg-gradient-to-br from-white via-[#FAFEFE] to-[#F3FBFB] border border-[#0E9F9F]/10 rounded-2xl shadow-[0_3px_10px_rgba(0,0,0,0.03)] p-6 transition-all duration-300 hover:shadow-[0_6px_20px_rgba(0,0,0,0.05)]">
            ...
            <div className="text-gray-600 leading-relaxed text-[15px] border-l-4 border-[#0E9F9F]/20 pl-4">
              {descItems.length > 1 ? (
                <ol className="list-decimal pl-5 space-y-2">
                  {descItems.map((line, idx) => (
                    <li key={idx}>{line}</li>
                  ))}
                </ol>
              ) : (
                <p>{description || "No description provided by seller."}</p>
              )}
            </div>
          </div>
          ...
          <div className="flex flex-wrap gap-6 text-sm text-gray-600 mt-6">
            ...
            <span className="flex items-center gap-1.5 hover:text-[#0E9F9F] transition-colors duration-200">
              <Eye size={16} className="text-[#0E9F9F]" />
              {ad.views || 0} views
            </span>

            <span className="flex items-center gap-1.5 hover:text-[#0E9F9F] transition-colors duration-200">
              <Heart size={16} className="text-[#0E9F9F]" />
...
<motion.div
  ...
  className="mt-12 bg-gradient-to-br from-[#F9FFFE] via-[#F4FAF9] to-[#EFFFFD] 
  border border-[#0E9F9F]/20 rounded-3xl p-6 
...
>
  ...
  <h3 className="font-semibold text-gray-800 flex items-center gap-2 mb-6 text-xl">
    <ShieldCheck className="text-[#0E9F9F]" size={22} />
    Seller Information
  </h3>
  ...
  <div className="flex flex-col gap-6 p-4 bg-white rounded-xl shadow-sm">
    ...
    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-6">
      ...
      >
        ...
        <div className="flex flex-col">
          <p className="font-semibold text-gray-900 text-lg leading-tight group-hover:text-[#0E9F9F] transition">
            {ad.ownerName || "Unknown Seller"}
          </p>

          {ad.ownerCity && (
            <p className="text-sm text-gray-500 mt-1 flex items-center gap-1">
              <MapPin size={14} className="text-[#0E9F9F]" />
              {ad.ownerCity}
            </p>
          )}
        </div>
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-4 border-t border-gray-100">
      ...
      <div className="flex items-center gap-3 bg-[#F0FAF9] p-4 rounded-xl">
        <Calendar size={20} className="text-[#0E9F9F]" />
        <div>
      ...
      {/* TOTAL ADS */}
      <div className="flex items-center gap-3 bg-[#F0FAF9] p-4 rounded-xl">
        <Layers size={20} className="text-[#0E9F9F]" />
        <div>
    ...
    {sellerStats?.totalAds >= 5 && (
      ...
      <motion.div
        ...
        className="relative bg-gradient-to-br from-[#FFFFFF] via-[#F9FEFE] to-[#E9FBFB] rounded-3xl shadow-[0_4px_20px_rgba(0,0,0,0.05)] p-8 mt-20 overflow-hidden"
      >
        {/* Decorative background glow */}
        <div className="absolute top-0 right-0 w-48 h-48 bg-[#0E9F9F]/10 rounded-full blur-3xl"></div>
        <div className="absolute bottom-0 left-0 w-40 h-40 bg-[#0E9F9F]/10 rounded-full blur-3xl"></div>

        {/* Header */}
        <div className="relative z-10 flex items-center gap-3 mb-6">
          <div className="p-3 bg-[#0E9F9F]/10 rounded-xl">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              className="h-7 w-7 text-[#0E9F9F]"
              fill="none"
              viewBox="0 0 24 24"
              stroke="currentColor"
            >
          ...
          <h2 className="text-2xl md:text-3xl font-semibold text-[#0E9F9F]">
      ...
      >
        ...
        <div className="flex items-center justify-between mb-8">
          <h2 className="text-3xl font-semibold text-[#0E9F9F]">
            You may also like
          </h2>
              ...
              >
                ...
                <div className="relative">
                    ...
                    src={
                      item.images?.[0]
                        ? item.images[0].startsWith("http")
                          ? item.images[0]
                          : toAssetUrl(item.images[0].replace(/\\/g, "/"))
                        : "https://cdn-icons-png.flaticon.com/512/4076/4076500.png"
                    }
                    alt={item.title}
                  ...
                  <div className="absolute inset-0 flex items-end justify-center pb-6 opacity-0 group-hover:opacity-100 transition-all duration-500">
                    <span className="px-4 py-2 rounded-full bg-white/90 text-[#0E9F9F] text-sm font-semibold shadow-lg backdrop-blur-sm transform translate-y-3 group-hover:translate-y-0 group-hover:scale-105 transition-all duration-500">
                      View Details
                    </span>
                  </div>
                </div>
                ...
                <div className="p-5">
                  ...
                  <div className="flex justify-between items-center mt-3">
                    <p className="text-[#0E9F9F] font-bold">
                      {item.category === "Jobs"
                        ? "Jobs"
          ...
          <div className="bg-white w-full max-w-md rounded-2xl p-6 shadow-xl">
            ...
            <div className="flex justify-end gap-3 mt-6">
              ...
              <button
                ...
                className="px-5 py-2 bg-[#0E9F9F] text-white rounded-lg hover:bg-[#0B7F7F]"
              >
                OK
              </button>
            </div>
          ...
          <div className="bg-white w-full max-w-2xl rounded-3xl p-8 shadow-xl">
            ...
            <div className="space-y-6">
              ...
              >
                ...
                <option value="Product already sold">Product already sold</option>
