...
const SearchResults = () => {
    ...
    const fetchResults = async () => {
      try {
        ...
        const res = await axios.get(
          `${API_BASE}/search/ads?q=${encodeURIComponent(
            searchQuery
  ...
  return (
    <section className="min-h-screen bg-gradient-to-b from-[#F1FFFC] to-white">
      {!loading && results.length > 0 && (
        <>
          {/* HERO HEADER */}
          <div className="relative px-4 sm:px-8 py-10 bg-gradient-to-r from-[#007E76] via-[#0E9F9F] to-[#007E76] text-white">
            <button
              onClick={() => navigate(-1)}
          ...
          <div className="sticky top-0 z-20 bg-white/70 backdrop-blur-xl border-b">
            <div className="max-w-6xl mx-auto px-4 sm:px-8 py-3 flex flex-wrap items-center gap-3 justify-between">
              <div className="flex flex-wrap items-center gap-3 text-sm">
                ...
                <span className="text-[#0E9F9F] bg-[#0E9F9F]/10 px-2.5 py-1 rounded-full text-xs font-semibold">
              ...
              <div className="flex items-center gap-3">
                {/* SORT PILLS */}
                <div className="flex gap-1 bg-transparent rounded-full p-1">
                  {SORTS.map((s) => (
                    <button
                      key={s}
                      onClick={() => setSortBy(s)}
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-8 py-10">
        ...
        {!loading && results.length === 0 && (
          <div className="text-center py-20">
            ...
            <button
              onClick={() => navigate("/")}
              className="px-6 py-2 rounded-lg bg-[#0E9F9F] text-white"
            >
              Back to Home
            </button>
          </div>
              ...
              >
                ...
                <div className="relative h-64 overflow-hidden">
                  ...
                  <span className="absolute bottom-3 left-1/2 -translate-x-1/2 opacity-0 group-hover:opacity-100 transition bg-white/90 px-4 py-1.5 rounded-full text-sm font-medium">
	                ...
	                {/* CONTENT */}
	                <div className="p-5">
	                  {!NO_PRICE_CATEGORIES.has(ad.category) && (
	                    <p className="text-lg font-semibold text-[#0E9F9F] mb-1">
                  ...
                  <div className="flex flex-wrap gap-2 mb-3">
                    {ad.category && (
                      <span className="text-xs bg-[#0E9F9F]/10 text-[#0E9F9F] px-2 py-1 rounded-full">
                        {ad.category}
                      </span>
                    )}
                    {ad.subCategory && (
            ...
            <button
              ...
              className={`px-4 py-2 rounded-full border text-sm font-semibold ${
                page === 1
                  ...
                  : "bg-white text-[#0E9F9F] border-[#0E9F9F]/30 hover:bg-[#0E9F9F]/10"
              }`}
            >
              Prev
            </button>
